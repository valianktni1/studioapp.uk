    ...
    <div className="fixed inset-0 z-[9999] bg-black flex items-center justify-center" data-testid="music-picker">
      ...
      <div className="text-center px-6 max-w-lg w-full">
        ...
        <div className="flex items-center justify-center gap-4 mb-3">
          <div className="h-px w-12" style={{ background: 'linear-gradient(to right, transparent, var(--brand))' }} />
          <Music className="w-5 h-5 text-[var(--brand)]" />
          <div className="h-px w-12" style={{ background: 'linear-gradient(to left, transparent, var(--brand))' }} />
        </div>
        ...
        <div className="space-y-3 mb-10">
          {TRACKS.map((track) => (
            <button
              ...
              style={{
                backgroundColor: selectedId === track.id ? 'rgba(var(--brand-rgb),0.12)' : 'rgba(255,255,255,0.04)',
                border: `1px solid ${selectedId === track.id ? 'rgba(var(--brand-rgb),0.4)' : 'rgba(255,255,255,0.08)'}`,
              }}
            >
              ...
              <div
                ...
                style={{
                  backgroundColor: previewingId === track.id ? 'rgba(var(--brand-rgb),0.3)' : 'rgba(255,255,255,0.08)',
                  border: `1px solid ${previewingId === track.id ? 'var(--brand)' : 'rgba(255,255,255,0.1)'}`,
                }}
              >
                {previewingId === track.id ? (
                  <Pause className="w-4 h-4 text-[var(--brand)]" />
                ) : (
              ...
              <div className="text-left flex-1">
                <p className="text-sm font-medium" style={{
                  ...
                  color: selectedId === track.id ? 'var(--brand)' : 'rgba(255,255,255,0.8)',
                }}>{track.name}</p>
              ...
              {/* Selection check */}
              {selectedId === track.id && (
                <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ backgroundColor: 'var(--brand)' }}>
        ...
        <div className="flex flex-col items-center gap-3">
          <button
            ...
            style={{
              fontFamily: 'Manrope, sans-serif',
              fontWeight: 600,
              backgroundColor: selectedId ? 'var(--brand)' : 'rgba(255,255,255,0.06)',
              color: selectedId ? '#1C1917' : 'rgba(255,255,255,0.25)',
              border: `1px solid ${selectedId ? 'var(--brand)' : 'rgba(255,255,255,0.08)'}`,
  ...
  return (
    ...
    >
      ...
      >
        <div className="text-center px-8">
          <div className="flex items-center justify-center gap-4 mb-6">
            <div className="h-px w-16" style={{ background: 'linear-gradient(to right, transparent, var(--brand))' }} />
            <span className="text-[var(--brand)] text-lg" style={{ lineHeight: 1 }}>&#10022;</span>
            <div className="h-px w-16" style={{ background: 'linear-gradient(to left, transparent, var(--brand))' }} />
          </div>
          ...
          <div className="absolute top-6 right-6 z-30 flex items-center gap-3"
            ...
            {shareToken && (
              <button data-testid="slideshow-share"
                ...
                className="w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md cursor-pointer"
                style={{
                  backgroundColor: linkCopied ? 'rgba(var(--brand-rgb),0.25)' : 'rgba(255,255,255,0.1)',
                  border: `1px solid ${linkCopied ? 'rgba(var(--brand-rgb),0.5)' : 'rgba(255,255,255,0.12)'}`,
                  transition: 'all 0.3s ease',
                }}
                title="Copy slideshow link">
                <Share2 className="w-4 h-4" style={{ color: linkCopied ? 'var(--brand)' : 'rgba(255,255,255,0.7)' }} />
              </button>
            )}
          ...
          {linkCopied && (
            <div className="absolute top-[72px] right-6 z-30 pointer-events-none"
              ...
              <div className="px-4 py-2 rounded-full backdrop-blur-xl"
                style={{ backgroundColor: 'rgba(0,0,0,0.5)', border: '1px solid rgba(var(--brand-rgb),0.3)' }}>
                <p className="text-xs tracking-wide" style={{ color: 'var(--brand)', fontFamily: 'Manrope, sans-serif' }}>
      ...
      <div className="absolute top-6 left-6 z-30 pointer-events-none"
        ...
        <p className="text-white/45 text-xs italic" style={{ fontFamily: 'Cormorant Garamond, serif' }}>{branding.business_name}</p>
      </div>
