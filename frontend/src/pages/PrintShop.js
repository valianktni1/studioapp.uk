    ...
    return (
      <div className="min-h-screen" style={{ backgroundColor: '#FDFCF8' }}>
        <div className="max-w-lg mx-auto px-6 py-12 text-center">
          ...
          <div className="border rounded-sm p-6 mb-6 text-left" style={{ borderColor: 'rgba(var(--brand-rgb),0.15)' }}>
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#FDFCF8' }}>
        ...
        <div className="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={scrollToCart} className="flex items-center gap-2 hover:bg-[#F5F2EB] px-3 py-2 rounded-sm transition-colors" data-testid="cart-header-btn">
              ...
              {cart.length > 0 && <span className="text-sm font-bold text-[var(--brand)]">£{subtotal.toFixed(2)}</span>}
            </button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-8 grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          {printSizes.length === 0 ? (
            ...
            <>
              <div className="mb-4 p-3 rounded-sm flex items-center gap-3" style={{ backgroundColor: 'rgba(var(--brand-rgb),0.08)', border: '1px solid rgba(var(--brand-rgb),0.2)' }}>
                <Printer className="w-5 h-5 text-[var(--brand)] flex-shrink-0" />
        ...
        <div className="lg:col-span-1" ref={cartRef}>
          <div className="sticky top-24 border rounded-sm p-4" style={{ borderColor: 'rgba(var(--brand-rgb),0.15)' }}>
            ...
            {cart.length > 0 && (
              <>
                <div className="border-t pt-3 space-y-1 text-sm" style={{ borderColor: 'rgba(var(--brand-rgb),0.15)' }}>
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {subtotal < MINIMUM_ORDER && cart.length > 0 && (
                    <div className="p-3 rounded-sm text-center" style={{ backgroundColor: 'rgba(var(--brand-rgb),0.1)', border: '1px solid rgba(var(--brand-rgb),0.3)' }}>
      ...
      {cart.length > 0 && (
        <button 
          ...
          className="lg:hidden fixed bottom-6 right-6 bg-[var(--brand)] text-white px-5 py-3 rounded-full shadow-lg flex items-center gap-2 z-50"
...
function PhotoCard({ file, galleryId, printSizes, onAdd }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} 
      className="border rounded-sm overflow-hidden" style={{ borderColor: 'rgba(var(--brand-rgb),0.15)' }}>
          ...
          {["gloss", "luster", "silk"].map(finish => (
            <button key={finish} onClick={() => setSelectedFinish(finish)}
              className={`flex-1 text-xs py-1.5 border capitalize ${
                selectedFinish === finish ? 'border-[var(--brand)] bg-[var(--brand)]/5 font-bold' : 'border-[#E5E5E5]'
              }`}>
              {finish}
            </button>
          ))}
